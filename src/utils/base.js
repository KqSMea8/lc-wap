...
export default {
    ...
    goNative(page,data,redirectUrl,extra={}){
        this.jsBridge({
            type: "goNative",
            data: {
                page:page,
                val:page=='login'?1:data,
                extra:extra,
    ...
    shareConfig: function (obj,action) {
        ...
        let data = obj || {
            ...
            share_url: !location.search?location.href:location.href.replace(location.search,'?')
        }
        if (isLecheng) {
            let shareData = {
                type: "share",
    ...
    activityRecord(to,activityInfo){
        
        for(const i in activityInfo){
            if(to.path.indexOf(i)!=-1){
                const target = activityInfo[i][1]?activityInfo[i][1].split('_')[0]:null;
                const targetId = activityInfo[i][1]?activityInfo[i][1].split('_')[1]:null;

                const taff = activityInfo[i][2]?activityInfo[i][2].split('_')[0]:null;
                const taffId = activityInfo[i][2]?activityInfo[i][2].split('_')[1]:null

                const inviter = activityInfo[i][3]?activityInfo[i][3].split('_')[0]:null;
                const inviterMbeId = activityInfo[i][3]?activityInfo[i][3].split('_')[1]:null

                const coupon = activityInfo[i][4]?activityInfo[i][4].split('_')[0]:null;
                const couponType = activityInfo[i][4]?activityInfo[i][4].split('_')[1]:null
                
                const extra=JSON.stringify({
                    eventType:1,
                    channel:activityInfo[i][0],
                    targetId:targetId?to[target][targetId]:null,
                    staffId:taffId?to[taff][taffId]:null,
                    inviterMbeId:inviterMbeId?to[inviter][inviterMbeId]:null,
                    couponType:couponType?to[coupon][couponType]:null,
                })
                return this.unbase64(extra)
            }
        }
    },
    unbase64(str){
        let base=window.btoa(encodeURIComponent(str))
