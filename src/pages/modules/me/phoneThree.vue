<template>
    <app-layout id="mePhoneTwo">
        <div class="title-box">
            ...
            <div class="item-box select" style="text-indent: .1rem;">换绑手机</div>
        </div>
        <div class="fill-box">
            <div class="input-box" style="margin-bottom:.05rem">
                <img src="~assets/img/me/icon_input_password@2x.png" alt="">
                <input type="text" placeholder="请输入新绑定的手机号码"  v-model="newphone" @keyup="keyUphone(newphone)">
            </div>
            <div class="input-box">
                <img src="~assets/img/me/icon-_input_message@2x.png" alt="">
                <input type="text" placeholder="请输入验证码" v-model="code" @keyup="keyUpnum(code)">
                <div class="code unclick" v-show="!effective" >获取验证码</div>
                <div class="code" v-show="effective&&!clickCode"  ref="sendcode">获取验证码</div>
                <div class="code unclick" v-show="clickCode">{{codeTime}}s</div>
            </div>
        </div>
        <div class="binding unclick" v-if="code==''">确定绑定</div>
        <div class="binding"  v-else @click="trueBInd">确定绑定</div>
            ...
            async trueBInd(){
                let d={
                    content: {
                        ...
                        oldPhone:this.$utils.encrypt(this.$cache.user.phone),
                        verifyCode:this.code
                    }
                };

                let res = await this.$post('/CRM/appmembermgcust/rebindMobilePhone.json',d);
                if(res.errcode==0){
                    this.$cache.user.phone=this.newphone;
                    this.$router.push({path:'/me/account'})
                }
            },
