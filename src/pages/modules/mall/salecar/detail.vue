<template>
  <app-layout id="carDetail" class="goodsDetail">
      ...
      <div class="goods-info">
        <div class="base">
          <div class="special-title">
            ...
            <div class="coupon">
              <p>优惠</p>
              <div>￥{{(parseFloat(pageData.zdprice)*10000-parseFloat(pageData.fixedprice)*10000).toFixed(0)}}</div>
            </div>
          </div>
          ...
          <div class="special">
            <h3>指导价：<del>￥{{parseFloat(pageData.zdprice)}}万</del></h3>
            <div class="special-price">
              <h2 style="display:flex;display:-webkit-flex;"><img class="icon-pricecar" src="~assets/img/mall/icon_detail_1pricecar.png"/><span style="font-size:.15rem">￥{{parseFloat(pageData.fixedprice)}}万</span></h2>
              <div style="color:#22222d">限量<span>{{parseFloat(pageData.store)}}</span>台</div>
            </div>
          </div>
        </div>
        ...
        <template v-if="pageData.status!=='kucunbuzu'&&pageData.status!=='xiajia'&&skus.length>0">
          ...
          <div class="arrow-link" :class="{'no-link':pageData.zitilist.length==1||isSwitch==0}" @click="openStore">
            <h1>提车门店</h1>
            <div v-if="selectedStore.id">{{selectedStore.title}}</div>
            <div v-else><span :class="youhuiNum>0?'youhuiTag':'choose'">{{youhuiNum>0?'门店活动':'请选择提车门店'}}</span></div>
          </div>
        </template>
        <car-discount :selectedStore="selectedStore" ></car-discount>
    ...
    <div slot="footer" class="btns">
      ...
      <div class="right">
        <template v-if="!['kucunbuzu','xiajia'].includes(pageData.status)">
          ...
          <button class="orange" @click="orderCar('order')">立即订车<br/><span style="font-size:.13rem">(订金:￥{{parseFloat(pageData.price)}})</span></button>
        </template>
        <button class="dis" v-if="pageData.status==='kucunbuzu'">售罄</button>
    ...
    <mt-popup slot="aside" v-model="showStores" position="bottom" style="width:100%;">
      <div class="fourS">
        <h1><span>共<em v-if="pageData.zitilist">{{pageData.zitilist.length}}</em>家门店</span>提车4S店</h1>
...
<script>
import carDiscount from 'components/orders/carDiscount'
export default {
  data() {
    return {
        storeId:null,
  ...
  activated() {
    if(!this.gid){
      ...
      this.isSwitch=this.$route.query.isSwitch||1,
      this.getPageData();
    }
  },
  ...
  methods: {
    ...
    openStore(){
      if(this.isSwitch==0||this.pageData.zitilist.length==1){
        return
      }
      // this.$login(()=>{
        this.showStores=true;
    ...
    selectStore(store){
      ...
      this.shareData.share_url=location.origin+'/wxwap?#'+this.$route.path+'?storeId='+store.crmid+'&isSwitch='+this.isSwitch;
      this.$utils.shareConfig(this.shareData);
    ...
    async getPageData () {
      ...
      if (res.errcode === 0) {
        ...
        if(spec){
            ...
            let fourS=res.content.zitilist,
                fi,
                stores={
                  city:[]
                };
            ...
            if(fourS.length==1){
              ...
              let arr=fourS.filter((item)=>{
                return item.crmid==this.storeId
              })
              
              if(arr.length>0){
                this.selectStore(arr[0])
              }
